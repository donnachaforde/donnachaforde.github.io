...
The constructor looked after the initialization of the two queues (lists) and determines which one is the initial target for the receiver, as follows:
...
Now, this approach uses exactly twice the memory size as before but we must remember a) our objective was to eliminate work for the GC and b) we're talking about containers that are merely holding references to event objects. In other words, in this case, we're not talking about large volumes of memory. 
